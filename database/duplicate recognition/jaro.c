...
/* The jaro function returns a double precision value from 0.0 (total
...
int jaro(PG_FUNCTION_ARGS)
{
	   text *arg1 = PG_GETARG_TEXT_P(0);
	   text *arg2 = PG_GETARG_TEXT_P(1);

	   char ying[VARSIZE(arg1) - VARHDRSZ], yang[VARSIZE(arg2) - VARHDRSZ];

	   strncpy( ying, VARDATA(arg1), VARSIZE(arg1) - VARHDRSZ );
	   strncpy( yang, VARDATA(arg2), VARSIZE(arg2) - VARHDRSZ );
	   long y_length = max(sizeof(ying), sizeof(yang));
	   int ind_c[] = {1,0};
...
if (!Num_com) PG_RETURN_FLOAT8(0.0);
